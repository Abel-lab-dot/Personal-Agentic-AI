...
import os
import traceback  # <--- NEW: For logging detailed stack traces

app = Flask(__name__)
CORS(app) # This allows your front-end to connect to the back-end

# Set your API key from an environment variable for security
# NOTE: The app crashes at startup if this key is missing.
# We confirmed the key is present because GET / is returning 200.
...
- Always use new lines to separate different points within a list or when transitioning to a new subtopic for improved readability.
- Use paragraph breaks (\\n\\n) to separate distinct sections, such as different roles or major accomplishments, to improve readability. Ensure that each new paragraph starts on a new line.
...
# *** FIX APPLIED HERE: Changed from 'gemini-1.5-flash-latest' to 'gemini-2.5-flash' ***
